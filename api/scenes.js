...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='GET'){
      ...
      const {data,error}=await client.from('ar_scenes').select('*').eq('owner_id',user.id).order('updated_at',{ascending:false}).limit(100)
    ...
    if(req.method==='POST'){
      ...
      const saved=await upsertScene(scene,{},user.id)
