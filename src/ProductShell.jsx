...
function BetaWorkspace({session,loading}){
 ...
 return <div className="betaWorkspace"><div className="betaBar"><span><b>ARQRAN BETA</b> {session?'Signed in — your workspace is account-linked.':'Guest mode — build freely; sign in when you want account-linked storage.'}</span>{!session&&<button onClick={()=>location.assign('/login?next=/studio')}>Sign in</button>}</div><SpatialPassthrough/></div>
}
